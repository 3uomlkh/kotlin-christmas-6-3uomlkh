...
- [X]  주문할 메뉴와 개수를 입력받는 기능(`{메뉴}-{갯수}` 형식)
...
- [X]  `<혜택 내역>` 이라는 문자열을 출력하는 기능
    - [X]  혜택 내역을 출력하는 기능
    - [X]  혜택 내역이 없을 경우 `없음` 이라는 문자열을 출력하는 기능
- [X]  `<총혜택 금액>` 이라는 문자열을 출력하는 기능
    - [X]  총 혜택 금액(할인 금액의 합계 + 증정 메뉴의 가격)을 출력하는 기능
- [X]  `<할인 후 예상 결제 금액>` 이라는 문자열을 출력하는 기능
    - [X]  할인 후 예상 결제 금액(할인 전 총주문 금액 - 할인 금액)을 출력하는 기능
- [X]  `<12월 이벤트 배지>` 라는 문자열을 출력하는 기능
    - [X]  12월 이벤트 배지를 출력하는 기능(별, 트리, 산타 중 한 개)
    - [X]  12월 이벤트 배지가 없을 경우 `없음` 이라는 문자열을 출력하는 기능
...
- [X]  날짜 분류 기능 테스트
- [X]  메뉴와 개수 파싱 기능 테스트
- [X]  할인 전 총주문 금액 계산 기능 테스트
- [ ]  총주문 금액에 따른 이벤트 판단 기능 테스트
- [X]  크리스마스 디데이 할인 적용 기능 테스트
- [X]  평일 할인 적용 기능 테스트
- [X]  주말 할인 적용 기능 테스트
- [X]  증정 이벤트 적용 기능 테스트
- [X]  총혜택 금액 계산 기능 테스트
- [X]  할인 후 예상 결제 금액 계산 기능 테스트
- [X]  12월 이벤트 배지 부여 기능 테스트
